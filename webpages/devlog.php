...
<p>Added starting music when (re-)entering the screen. Fixed playlist loading (had audio turned off during development, so I didn't notice this until now). Audio tracks can now referenced with an easier-to-remember enum.
</p>
...
<b>2003-04-30</b>
<p>
Added Radoslaw Grzanka "radekg2" to the project; he is responsible for debian packaging. Released his .deb packge for the 0.8a-hot-chili release. Started discussion with John McCawley about AI implementation.
</p>
...
<p>
Modified the Lua library in such a way that it can parse hex digits, like 0x016e, which greatly simplifies work with item_id's throughout the code. Maybe I should send the <a href="http://cvs.sourceforge.net/cgi-bin/viewcvs.cgi/uwadv/uwadv/source/lua/lua-hexdigit-patch.diff?rev=HEAD&amp;content-type=text/vnd.viewcvs-markup">hexdigit patch</a> to the lua-users list or so. Modified all Lua scripts to use the new format when possible and adquate. Moved the Lua and zziplib library files to the uaopt project to get some space in msvc and optimized code.
</p>
...
I'm very proud to annouce some fantastic news today! The Underworld Adventures Team got the original sourcecode to Ultima Underworld 1 and 2 today! Doug Church himself, the lead developer for both games, sent a zipped file with the game's source code. And the best is: we, the team, offer you the download, too (see below).
</i>
</p>
...
<p>Factored out the panel on the right of the user interface - the inventory/stats/runebag control. Added hack to get bridges to work, rendering and collision detection wise. Had to use an ugly hack to access the model manager from the physics code (static pointer to mgr). Also started to rewrite the uw-formats.txt file in a more organized manner.
</p>
...
<p>Ah, and added pausing the underworld. Oh, and fixed the lua_savegame_* functionality. And disabled the "level up" and "level down" keys for release builds. I hope level changing by triggers/traps is possible 'till the next release.
</p>
...
<p>
The people at <a href="http://www.rpgcodex.com/" target="_blank">RPGCodex</a> also discovered uwadv. They even added the screenshow with the "abyss cubus" :) And yes, people seem to still have Voodoo3's :) Another <a href="http://www.jokerbone.com/archives/2002/11/08/000766.html#000309" target="_blank">site</a> (a blog writer) even titled his article "Mages get the chicks..." :)
</p>
...
<b>2003-01-29</b>
<p>
Replaced the way the debugger accessed infos of the underworld object with a new one. The old one was done using a class, and that wasn't that portable between msvc and mingw. The new system uses a single callback "command" function that the debugger can call to get infos. It can be given a command, a command typ and two in/out-parameters using a union inside a struct (should be portable enough). Tried to get the wxWindows stuff to compile on mingw32, but there still are some problems. Added a "Underworld Adventures features" check to the uaconfig program.
</p>

<b>2003-01-28</b>
<p>
Addd support for translation, rotation and scale keywords from vrml. Don't know if I handle them properly. Enough with 3d models for now.
</p>
...
<b>2002-11-25</b>
<p>
Added ua_image_quad class to encapsulate rendering of big images using only textures up to 256x256 (functionality was moved from the ua_textscroll class). Used the class for 2d images in text scroll, spash screen, as base class for ua_cutscene, acknowledgement screen and the start menu. uwadv should be Voodoo3-safe now. Also tried to unpack uw2 conversations (they used a simple compression scheme this time).
</p>
...
<b>2002-11-20</b>
<p>
Added looking at objects in 3d world and fixed some last bugs. Fixed rendering ob 2d critter sprites. Added some code to read in the models from the executable (not used for release). Fixed doxygen documentation generation and updated the uw-formats.txt with 3d models info. FMOD support was ripped away.
</p>
...
<b>2002-10-16</b>
<p>
Invented a underworld access API for access of uadebug to the ua_underworld object and its members. Player Info in the debuggger is filled now (unfortunately, the uadebug.dll can't be used with mingw32). Also added usage of precompiled headers to uadebug, which speeds up compilation quite a bit. Mailed the author of the <a href="http://www.anthemion.co.uk/wxworkshop/docs/html/fl/fl.htm" target="_blank">fl library</a> (it's part of the new wxWindows beta release) used about a possible bug.
</p>
...
<b>2002-07-05</b>
<p>
tinkering with collision detection, but nothing to commit yet. improved underworld object and uw1 game importing while while working on the game physics. today I also noticed that Underworld Adventures is part of <a href="http://sourceforge.net/foundry/games/" target="_blank">SourceForge's Games Foundary</a>.
</p>
...
<b>2002-06-12</b>
<p>
I took a few days off from uwadv, to finish <a href="http://winLAME.sourceforge.net/" target="_blank">another project</a>. I also thought about how to integrate the scriping language <a href="http://www.lua.org/" target="_blank">Lua</a> into uwadv, to manage ingame actions and NPC behaviour, as well as character creation or cutscenes control.
</p>
