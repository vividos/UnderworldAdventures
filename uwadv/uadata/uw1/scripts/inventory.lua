...
inv_cmb_list = {
   ...
   { item_id1 = 0x0095, item_id2 = 0x0116, result_id = 0x0115,
     success_code = lua_inv_cmb_dstr_second },
   ...
   { item_id1  = 0x011c, item_id2 = 0x00d8, result_id = 0x012b,
     success_code = lua_inv_cmb_dstr_both },
...
function lua_inventory_is_container(item_id)
   ...
   if item_id >= 0x0080 and item_id <= 0x008e
   then
      ret = 1
   end
...
function lua_inventory_categorize_item(item_id)
   ...
   if item_id >= 0x0033 and item_id <= 0x003a and item_id ~= 0x0037
   then
      type = lua_inv_cat_ring
   end

   -- head item; [0x002c; 0x002e] and [0x0030; 0x0032]
   if (item_id >= 0x002c and item_id <= 0x002e) or
      (item_id >= 0x0030 and item_id <= 0x0032)
   then
      type = lua_inv_cat_head
   end

   -- chest item; [0x0020; 0x0022]
   if item_id >= 0x0020 and item_id <= 0x0022
   then
      type = lua_inv_cat_chest
   end

   -- hands item; [0x0026; 0x0028]
   if item_id >= 0x0026 and item_id <= 0x0028
   then
      type = lua_inv_cat_hands
   end

   -- legs item; [0x0023; 0x0025]
   if item_id >= 0x0023 and item_id <= 0x0025
   then
      type = lua_inv_cat_legs
   end

   -- feet item; [0x0029; 0x002b] and { 0x002f }
   if (item_id >= 0x0029 and item_id <= 0x002b ) or item_id == 0x002f
   then
