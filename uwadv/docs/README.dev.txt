...
1.1. Used libraries
   ...
   * OpenGL 1.1 (or greater), http://www.opengl.org/
...
1.3. doxygen use
   ...
   Throughout the code, doxygen-style comments for functions, typedefs,
   structs and files are used. To create the doxygen source code
   documentation, change into the "docs" folder and type:
...
1.4. CVS use
   ...
   CVS, using the SourceForge CVS repository services. You can anonymously
   access the repository via:
   ...
   Once the project files are checked out and a CVS folder does exist, the -d
...
2. Basic uwadv stuff
   ...
   This chapter explains some basic mechanics of the uwadv project. From here
   on it is is useful if you know the original game's file formats, as
   described in "uw-formats.txt". It is assumed that the reader has some
