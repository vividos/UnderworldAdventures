...
void ua_textscroll::init(IGame& game, unsigned int xpos,
...
bool ua_textscroll::process_event(SDL_Event& event)
{
      ...
      (event.key.keysym.sym == SDLK_SPACE || event.key.keysym.sym == SDLK_RETURN))
   {
      ...
      first_visible_line += std::min(textlines.size() - first_visible_line - maxlines, maxlines);
...
void ua_textscroll::update_scroll()
{
   ...
   unsigned int max = std::min(maxlines, textlines.size());
...
void ua_textscroll::create_colored_string(IndexedImage& img, const char* text)
{
   std::string line(text);
