...
void GameLogic::EvaluateUnderworld(double time)
{
   ...
   if (m_isAttacking)
   {
      ...
      if (m_userInterface != NULL)
         m_userInterface->uw_notify(::ua_notify_update_powergem);
   }
...
void GameLogic::UserAction(EUserInterfaceUserAction action,
   unsigned int param)
{
   if (m_scripting == NULL)
      return;
   ...
   case ua_action_combat_release:
      ...
      UaTrace("m_isAttacking with power=%u\n", m_attackPower);
...
void GameLogic::ChangeLevel(unsigned int level)
{
   ...
   if (m_userInterface != NULL)
      m_userInterface->uw_notify(::ua_notify_level_change);
