...
void IngameCompass::Init(IGameInstance& game, unsigned int xpos,
   unsigned int ypos)
{
   // init image
...
void IngameCompass::Draw()
{
   ...
   Underworld::Player& player = m_gameInstance.GetUnderworld().
      GetPlayer();
   ...
   bool buttonDown, unsigned int mouseX, unsigned int mouseY)
{
   ...
   {
      m_gameInstance.GetGameLogic().UserAction(userActionClickedCompass);
      return true;
   }
