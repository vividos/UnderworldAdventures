...
void ua_create_character_screen::init()
{
   ...
   pplayer = &(game.GetUnderworld().GetPlayer());
...
bool ua_create_character_screen::process_event(SDL_Event& event)
{
   ...
   case SDL_KEYDOWN:
      ...
      case SDLK_DOWN:
         ...
         if (selected_button + 1 < static_cast<int>(btng_buttoncount))
            selected_button++;
         break;

      case SDLK_RIGHT:
         // select the button to the right, if possible
         if (selected_button + btng_buttonspercolumn < static_cast<int>(btng_buttoncount))
...
void ua_create_character_screen::do_action()
{
   ...
   IndexedImage& img = img_screen.get_image();
   ...
   case actEnd:
      ...
      // fade out music
      game.get_audio_manager().FadeoutMusic(ua_fade_time);
   ...
   case actSetInitVal:
   {
      ...
      if (game.get_settings().GetBool(Base::settingUwadvFeatures))
   ...
   case actSetUIBtnGroup:
   {
      ...
      else
      {
         ...
         for (int i = 0; i < static_cast<int>(btng_buttoncount); i++)
         {
            lua_rawgeti(L, 4, i + 1);
   ...
   {
      if (n < 4) break;
      IndexedImage& cimg = img_buttons[static_cast<unsigned int>(lua_tonumber(L, 2))];
   ...
   case actSetPlayerName:
      if (n < 2) break;
      pplayer->SetName(lua_tostring(L, 2));
      //UaTrace("player name set to \"%s\" by char. creation script\n", lua_tostring(L,2));
      break;

   case actSetPlayerAttr:
      if (n < 3) break;
      pplayer->SetAttribute(static_cast<Underworld::EPlayerAttribute>(static_cast<unsigned int>(lua_tonumber(L, 2))),
         static_cast<unsigned int>(lua_tonumber(L, 3)));
   ...
   case actSetPlayerSkill:
      if (n < 3) break;
      pplayer->SetSkill(static_cast<Underworld::EPlayerSkill>(static_cast<unsigned int>(lua_tonumber(L, 2))),
         static_cast<unsigned int>(lua_tonumber(L, 3)));
...
{
   std::string text(game.GetGameStrings().GetString(custstrblock > -1 ? custstrblock : strblock, strnum));
...
{
   IndexedImage button = img_buttons[btng_buttonimg_normal];
...
void ua_create_character_screen::drawbuttongroup()
{
   ...
   if (iex != static_cast<int>(btng_buttoncount))
