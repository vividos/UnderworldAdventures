...
void ua_ingame_new_screen::init()
{
   ...
   ua_trace("new ingame user interface started\n");
...
void ua_ingame_new_screen::destroy()
{



   ua_trace("new ingame user interface finished\n\n");
}
...
bool ua_ingame_new_screen::process_event(SDL_Event& event)
{
   return ua_screen::process_event(event);
}

void ua_ingame_new_screen::key_event(bool key_down, ua_key_value key)
{
...
void ua_ingame_new_screen::tick()
{
   // evaluate underworld
...
void ua_ingame_orig_screen::resume()
{
   ...
   if (fadeout_action == 3)
   {
      ...
      core->get_audio().start_music(ua_music_uw1_dark_abyss,true);
   }
   else
   {
      // normal start, play "Descent"
      core->get_audio().start_music(ua_music_uw1_descent,true);
   }
}
      ...
      ++fade_ticks >= (core->get_tickrate()*fade_time))
   {
      ...
      if (fade_state==3)
      {
         ...
         case 1: // start "save game" screen
            core->push_screen(new ua_save_game_screen(game,false));
            break;
         ...
         case 3: // start conversation
            core->push_screen(new ua_conversation_screen(game,conv_level,conv_objpos));
            break;

         case 4: // start cutscene
            // show cutscene
            core->push_screen(new ua_cutscene_view_screen(game,fadeout_param));
            break;
         }
      }
   }
