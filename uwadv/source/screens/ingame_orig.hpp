...
class ua_ingame_orig_screen : public ua_screen, public IUserInterface,
   public ua_panel_parent_interface
{
public:
   /// ctor
   ua_ingame_orig_screen(IGame& game);
   /// dtor
   ...
   virtual void key_event(bool key_down, Base::EKeyType key) override;
   ...
   virtual IGame& get_game_interface() { return game; }
...
protected:
   ...
   virtual void uw_print(const char* text) override;

   /// starts conversation with object in current level, on list position
   virtual void uw_start_conversation(Uint16 list_pos) override;
...
protected:
   ...
   ua_player_physics_object m_playerPhysics;
};
