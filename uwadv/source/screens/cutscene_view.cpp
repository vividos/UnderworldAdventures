...
void ua_cutscene_view_screen::init()
{
   ...
   // stop audio track
   game.get_audio_manager().StopMusic();
...
void ua_cutscene_view_screen::destroy()
{
   ...
   // stop audio track
   game.get_audio_manager().StopSound();
...
void ua_cutscene_view_screen::tick()
{
   ...
   if (showanim && loopanim && !(anim_fade_state == 1 || anim_fade_state == 2))
   {
      ...
      if (animcount >= 1.0 / anim_fps)
      {
         ...
         if (stopframe >= 0 && curframe == static_cast<unsigned int>(stopframe))
         {
...
void ua_cutscene_view_screen::create_text_image(const char* str)
{
   ...
   else
   {
      ...
      // create new image
      {
         ...
         IndexedImage& img = img_text.get_image();
...
void ua_cutscene_view_screen::do_action()
{
   ...
   case 2: // cuts_sound_play
      if (canplaysound)
      {
         ...
         game.get_audio_manager().PlaySound(str);
      }
      break;
   ...
   case 4: // cuts_text_fadein
      if (canshowtext)
      {
         ...
         create_text_image(game.GetGameStrings().GetString(strblock, strnum).c_str());
