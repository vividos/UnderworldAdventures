...
void ua_image_draw_edge(IndexedImage& img, unsigned int xpos, unsigned int ypos,
   ...
   const char* text, ua_save_game_button_id my_id)
{
   ...
   if (game.get_settings().GetBool(Base::settingUw1IsUwdemo))
   {
      ...
      Base::String::Uppercase(button_text);
   }
   else
   {
...

   {
      ...
      int text_xpos = (signed(button_width) - signed(img_temp.get_xres())) / 2;
      if (text_xpos < 0)
         text_xpos = 0;
      unsigned int text_ypos = (16 - img_temp.get_yres()) / 2;

      img_buttons[1].paste_image(img_temp, text_xpos, text_ypos, true);

      // highlighted
      font_normal.create_string(img_temp, button_text.c_str(), 73);

      text_xpos = (signed(button_width) - signed(img_temp.get_xres())) / 2;
      if (text_xpos < 0)
         text_xpos = 0;

      img_buttons[2].paste_image(img_temp, text_xpos, text_ypos, true);
   }
...
void ua_save_game_button::update_button(bool state_pressed)
{
   ...
   IndexedImage& button = img_buttons[state_pressed ? 1 : 2];
...
void ua_save_game_savegames_list::update_list()
{
   ...
   unsigned int numitems = savegames_manager->GetSavegamesCount() + (show_new ? 0 : 1);
   ...
   for (unsigned int i = list_base; i < numitems; i++)
   {
      ...
      else
      {
         ...
         savegames_manager->GetSavegameInfo(i, info);
...
ua_save_game_screen::ua_save_game_screen(IGame& game,
      ...
      if (fade_state == 3)
      {
         ...
         case ua_button_load:
         {
            UaTrace("loading saved game, filename %s\n",
               game.get_savegames_manager().GetSavegameFilename(
            ...
            // load savegame
            Base::Savegame sg = game.get_savegames_manager().GetSavegameFromFile(
               game.get_savegames_manager().GetSavegameFilename(
                  savegames_list.get_selected_savegame()).c_str());
...
void ua_save_game_screen::press_button(ua_save_game_button_id id)
{
   ...
   case ua_button_load:
   {
      ...
      if (savegames_list.get_selected_savegame() == -1 ||
         static_cast<unsigned int>(savegames_list.get_selected_savegame()) >=
         game.get_savegames_manager().GetSavegamesCount())
         break;
...
void ua_save_game_screen::update_info()
{
   ...
   IndexedImage& img_info = img_infoarea.get_image();

   // restore original image
   img_info.clear(0);

   if (selected_savegame >= 0 &&
      static_cast<unsigned int>(selected_savegame) < game.get_savegames_manager().GetSavegamesCount())
   {
      // get savegame infos
      Base::SavegameInfo info;
      game.get_savegames_manager().GetSavegameInfo(selected_savegame, info);
      ...
      // player name
      unsigned int width = font_btns.calc_length(info.title.c_str());
      unsigned int xpos = width > 128 ? 0 : (128 - width) / 2;
      font_btns.create_string(img_temp, info.title.c_str(), 162);
      img_info.paste_image(img_temp, xpos, 2, true);
      img_temp.clear();

      // gender
      std::string text(game.GetGameStrings().GetString(2, info.gender + 9));
      font_btns.create_string(img_temp, text.c_str(), textcolor);
      img_info.paste_image(img_temp, 2, 13, true);
      img_temp.clear();

      // profession
      text = game.GetGameStrings().GetString(2, info.profession + 23);
      font_btns.create_string(img_temp, text.c_str(), textcolor);
      ...
      {
         // text
         text = game.GetGameStrings().GetString(2, i + 17);
         font_btns.create_string(img_temp, text.c_str(), textcolor);
      ...
      #ifdef HAVE_DEBUG
            // savegame filename
            text = sgmgr->GetSavegameFilename(selected_savegame);
      ...
      */
      // do preview image
      tex_preview.init();
      tex_preview.convert(info.imageXRes, info.imageYRes, &info.imageRGBA[0]);
      tex_preview.upload();
...
void ua_save_game_screen::ask_savegame_desc()
{
   ...
   Base::SavegamesManager& sgmgr = game.get_savegames_manager();

   // select new slot when no savegame was selected
   int selected_savegame = savegames_list.get_selected_savegame();
   if (selected_savegame != -1 && static_cast<unsigned int>(selected_savegame) < sgmgr.GetSavegamesCount())
   {
      // retrieve savegame title
      Base::SavegameInfo info;
      sgmgr.GetSavegameInfo(selected_savegame, info);
      savegame_name = info.title;
   }
   else
      selected_savegame = sgmgr.GetSavegamesCount();
...
void ua_save_game_screen::save_savegame()
{
   ...
   game.GetUnderworld().GetPlayer().FillSavegamePlayerInfos(info);
   ...
   if (selected_savegame != -1 && static_cast<unsigned int>(selected_savegame) < sgmgr.GetSavegamesCount())
   {
      UaTrace("saving game over old savegame slot, filename %s\n",
         sgmgr.GetSavegameFilename(selected_savegame).c_str());

      // saving over selected game
      Base::Savegame sg = sgmgr.SaveSavegame(info, selected_savegame);
