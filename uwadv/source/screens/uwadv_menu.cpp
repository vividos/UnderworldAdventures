...
void ua_uwadv_menu_screen::init()
{
   ...
   // load texture
   Base::SDL_RWopsPtr rwops = game.GetResourceManager().GetResourceFile("uwadv-loading.tga");
...
void ua_uwadv_menu_screen::tick()
{
   ...
   /*
      paste one:
      --------------------
      game.GetUnderworld().import_savegame(game.get_settings(),"data/",true);
      ...
      --------------------
      game.GetUnderworld().import_savegame(game.get_settings(),"data/",true);
      game.GetUnderworld().get_scripts().lua_started_newgame();
      game.replace_screen(new ua_ingame_orig_screen(game),false);
      --------------------
   */
#else
