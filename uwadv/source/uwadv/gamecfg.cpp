...
void ua_gamecfg_loader::Load(Base::TextFile& file)
{
...
bool ua_gamecfg_loader::SplitKeyValue(std::string& line, std::string& key, std::string& value)
{
   if (line.empty())
      return false;
...
void ua_gamecfg_loader::load_value(const char* the_name, const char* the_value)
{
   ...
   else if (name.compare("init-scripting") == 0)
   {
      ...
      {
         // init Lua scripting
         *scripting = IScripting::create_scripting(sriptingLanguageLua);
