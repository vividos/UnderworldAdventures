...
void ua_texture::convert(IndexedImage& img, unsigned int numtex)
{
...
void ua_texture::convert(Uint8* pix, unsigned int origx, unsigned int origy,
   Palette256& pal, unsigned int numtex)
{
...
void ua_texture::upload(unsigned int numtex, bool mipmaps)
{
   ...
   if (error != GL_NO_ERROR)
      UaTrace("ua_texture: error during uploading texture! (%u)\n", error);
}
...
void ua_texture_manager::init(IGame& game)
{
   ...
   if (settings.GetGametype() == Base::gameUw1)
   {
      ...
      std::string walltexfname(settings.GetString(Base::settingUnderworldPath));
      walltexfname.append(
         settings.GetBool(Base::settingUw1IsUwdemo) ? "data/dw64.tr" : "data/w64.tr");

      Import::TextureLoader::LoadTextures(allstocktex_imgs, ua_tex_stock_wall,
         walltexfname.c_str(), palette0);

      // load all floor textures
      std::string floortexfname(settings.GetString(Base::settingUnderworldPath));
      floortexfname.append(
         settings.GetBool(Base::settingUw1IsUwdemo) ? "data/df32.tr" : "data/f32.tr");

      Import::TextureLoader::LoadTextures(allstocktex_imgs, ua_tex_stock_floor,
   ...
   {
      // load all textures
      std::string texfname(settings.GetString(Base::settingUnderworldPath));
