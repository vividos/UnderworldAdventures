...
void ua_renderer::init(IGame& game, SDL_Window* window)
{
   m_window = window;

   renderer_impl = new ua_renderer_impl;
   if (renderer_impl == NULL)
      throw Base::Exception("couldn't create ua_renderer_impl class");
   ...
   unsigned int width, unsigned int height)
{
   ...
   viewport[0] = xpos;
   viewport[1] = windowHeight - ypos - height;
   viewport[2] = width;
   viewport[3] = height;
}
...
void ua_renderer::render_underworld(const Underworld::Underworld& underw)
{
   const Underworld::Player& pl = underw.GetPlayer();

   double plheight = 0.6 + pl.GetHeight();
   ua_vector3d pos(pl.GetXPos(), pl.GetYPos(), plheight);

   pos += view_offset;

   // render map
   renderer_impl->render(underw.GetCurrentLevel(), pos, pl.GetPanAngle(),
      pl.GetRotateAngle(), fov);
}
...
void ua_renderer::select_pick(const Underworld::Underworld& underw, unsigned int xpos,
   ...
   unsigned int& id)
{
   ...
   // set up projection matrix for selection rendering
   {
      ...
      gluPickMatrix(xpos, windowHeight - ypos, 5.0, 5.0, viewport);
...
void ua_renderer::prepare_level(Underworld::Level& level)
{
   ...
   {
      const std::set<Uint16>& used_textures = level.GetTilemap().GetUsedTextures();

      for (Uint16 textureId : used_textures)
         texmgr.prepare(textureId);
   }
   ...
   get_critter_frames_manager().prepare(&level.GetObjectList());
   ...
   std::vector<ua_triangle3d_textured>& alltriangles)
{
   ...
   if (model_mgr.model_avail(obj.GetObjectInfo().m_itemID))
   {
