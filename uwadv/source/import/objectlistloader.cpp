   ...
   // object modifications
   {
      ...
      if (itemID == 0x0164)
      {
         if (objInfo.m_flags < 2)
            objInfo.m_flags = (30 + objInfo.m_flags + Base::c_uiStockTexturesTmobj);
         else
...
void ObjectListLoader::AddNpcInfos(Underworld::ObjectPtr& obj, Uint8* npcInfos)
{
   ...
   npcInfo.npc_goal = static_cast<Uint8>(GetBits(value03, 0, 3));
   npcInfo.npc_gtarg = static_cast<Uint8>(GetBits(value03, 4, 8));

   Uint16 value05 = npcInfos[0x0005] | (npcInfos[0x0006] << 8);
   npcInfo.npc_level = static_cast<Uint8>(GetBits(value05, 0, 3));
   npcInfo.npc_talkedto = GetBits(value05, 13, 1) == 1;
   npcInfo.npc_attitude = static_cast<Uint8>(GetBits(value05, 14, 2));

   Uint16 value0E = npcInfos[0x000e] | (npcInfos[0x000f] << 8);
   npcInfo.npc_xhome = static_cast<Uint8>(GetBits(value0E, 10, 6));
   npcInfo.npc_yhome = static_cast<Uint8>(GetBits(value0E, 4, 6));

   npcInfo.npc_whoami = npcInfos[0x0012];
}
